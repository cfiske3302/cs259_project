...
# make it understand Qwen2-VL's text backbone (Qwen2VLTextModel).
...
def _get_blocks_patched(model):
    """Extend AWQ's get_blocks to handle Qwen2VLTextModel.

    For Qwen2-VL, the language backbone is `Qwen2VLTextModel`, which owns
    `layers` (a ModuleList of decoder layers). We quantize ONLY this text
    model, not the vision encoder.
    """
    name = model.__class__.__name__

    # Text backbone of Qwen2-VL
...
def _move_embed_patched(model, device: str):
    """Extend AWQ's move_embed to handle Qwen2VLTextModel.
    ...
    if name == "Qwen2VLTextModel":
        # Qwen2VLTextModel defines `embed_tokens` and `rotary_emb` at the
...
def parse_args():
    parser = argparse.ArgumentParser(
        description="Run AWQ search on the Qwen2-VL-2B-Instruct text backbone and save awq_results."
    )
    parser.add_argument(
        "--model_path",
        type=str,
        default="Qwen/Qwen2-VL-2B-Instruct",
        help="Hugging Face repo ID or local path for Qwen2-VL-2B-Instruct.",
    )
    parser.add_argument(
        "--output_path",
        type=str,
        default="awq_cache/qwen2-vl-2b-instruct-w4-g128.pt",
...
def main():
    ...
    print(f"Loading Qwen2-VL model from {args.model_path} (dtype={torch_dtype}) ...")
    ...
    # Extract the language model part: Qwen2VLTextModel
    ...
    print(
        f"Running AWQ search on Qwen2-VL text model with "
